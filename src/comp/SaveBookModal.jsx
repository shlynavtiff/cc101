...
const SaveBookModal = ({ objectItem, closer }) => {
          ...
          setFilteredArr(prev =>
            prev.map(item =>
              item.Uid === user?.uid
                ? {
                    ...item,
                    MyBook: Array.isArray(item.MyBook) ? [...item.MyBook] : []
                  }
                : item
            )
          )
      ...
      try {
        ...
        if (existingUserDoc) {
          ...
          const updatedMyCollection = existingUserDoc
            .data()
            .MyBook.filter(item => item.id !== objectItem.id)
  ...
  const isItemSaved = useCallback(() => {
    return filteredArr.some(userItem => {
      if (userItem?.MyBook) {
        return userItem.MyBook.some(
          collectionItem => collectionItem.id === objectItem.id
        )
      }
    })
